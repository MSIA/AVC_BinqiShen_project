...
model:
	docker run --mount type=bind,source="$(shell pwd)",target=/app/ application_data run.py run_model_pipeline

all: image upload_data_s3 download_data_s3 create_db_local create_db_rds connect_db model
